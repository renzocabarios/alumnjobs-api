...
const getAll = async (_req, _res) => {
  ...
  _res.send({ data, status: "success", message: "Get jobs success" });
};

const getById = async (_req, _res) => {
  const { id } = _req.params;
  const data = await service.getById(id);
  _res.send({ data: [data], status: "success", message: "Get jobs success" });
};
